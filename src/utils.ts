function includes<T>(arr: T[], searchElement: T) {
    return arr.some(element => element === searchElement);
}

// <SP> <TAB> <VT> <FF> <NBSP> <ZWNBSP>
// <USP>
export function isWhiteSpace(cp: number): boolean {
    return includes([0x20, 0x09, 0x0B, 0x0C, 0xA0, 0xFEFF], cp) ||
    (cp >= 0x1680 && cp <= 0x3000 && ((cp >= 0x2000 && cp <= 0x200A)
    || includes([0x1680, 0x202F, 0x205F, 0x3000], cp)));
}
...
// Some helpful functions when dealing with javascript
// Defined in namespace JS
// ECMAScript@2017
export const JS = {
    ...
    return (cp === 0x24) || (cp === 0x5F) ||  // $ (dollar) and _ (underscore)
        ...
        (cp >= 0x61 && cp <= 0x7A) ||         // a..z
        (cp === 0x5C) ||                      // \ (backslash)
        ((cp >= 0x80) && isInRanges(cp, NonAsciiIdentifierStartRanges));
    },
    ...
    return (cp === 0x24) || (cp === 0x5F) ||  // $ (dollar) and _ (underscore)
        ...
        (cp >= 0x30 && cp <= 0x39) ||         // 0..9
        (cp === 0x5C) ||                      // \ (backslash)
        ((cp >= 0x80) && isInRanges(cp, NonAsciiIdentifierPartRanges));
    },
